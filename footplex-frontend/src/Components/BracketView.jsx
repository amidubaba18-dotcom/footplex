                            ...
                            <div className="flex flex-col gap-6 justify-around" style={{ minHeight: `${Math.max(round.matches.length, 1) * 92}px` }}>
                                {round.matches.map(match => (
                                    <MatchCard
                                        key={match.id}
                                        match={match}
                                        accentClass={title === 'Grand Finals' ? 'border-yellow-200' : 'border-gray-200'}
                                    />
                                ))}
                            </div>
                        </div>
...
function getRounds(matches) {
    return [...new Set(matches.map(match => match.round_number))]
        .sort((a, b) => a - b)
        .map(roundNumber => ({
            roundNumber,
            matches: matches.filter(match => match.round_number === roundNumber)
        }))
}
...
export default function BracketView({ fixtures = [], tournament }) {
    ...
    const knockoutFixtures = tournament?.format === 'group_knockout'
        ? fixtures.filter(match => !match.group_name && match.match_type === 'knockout')
        : fixtures

    if (tournament?.format === 'group_knockout' && knockoutFixtures.length === 0) {
        return (
            <div className="bg-white rounded-xl border border-gray-200 p-12 text-center text-gray-400 text-sm">
    ...
    if (!isDoubleElim) {
        const singleElimMatches = knockoutFixtures.filter(match => match.match_type === 'knockout')
        const bracketMatches = singleElimMatches.length > 0 ? singleElimMatches : knockoutFixtures
        const rounds = getRounds(bracketMatches)
        const totalMatches = bracketMatches.length
        const completedMatches = bracketMatches.filter(match => match.status === 'completed').length
            ...
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                ...
                <div className="p-6">
                    <BracketSection
                        title="Knockout"
                        rounds={rounds}
                        roundName={getKnockoutRoundName}
                    />
                </div>
            </div>
        )
    ...
    const winnersRounds = getRounds(fixtures.filter(match => match.match_type === 'winners'))
    const losersRounds = getRounds(fixtures.filter(match => match.match_type === 'losers'))
    const finalsRounds = [
        ...getRounds(fixtures.filter(match => match.match_type === 'grand_final')),
        ...getRounds(fixtures.filter(match => match.match_type === 'grand_final_reset'))
            .map(round => ({ ...round, roundNumber: round.roundNumber + 1 }))
    ]
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div className="p-6 space-y-8">
                <BracketSection
                    ...
                    roundName={roundNumber => `Round ${roundNumber}`}
                />
                ...
                <BracketSection
                    title="Grand Finals"
                    rounds={finalsRounds}
                    roundName={roundNumber => roundNumber === 1 ? 'Grand Final' : 'Reset Final'}
